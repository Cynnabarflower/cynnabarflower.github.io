new myMap('123', JSON.parse(JSON.stringify(clean_map_data)), 'maps_place', map_values)

ymaps.ready(function(){
    init_yaMap(3, 5, 1, 1/1.0).then(ya_map_data => {
        new myMap('666', ya_map_data, 'maps_place', map_values);
    });
});
...
function myMap(mapId, map_data, parent_id, map_values) {
    var
...
/*
        ...
        if (!('max_val' in map_values) || !('min_val' in map_values)) {
            max_val = -99999999;
            min_val = 99999999;
            for (shortname in map_values) {
                for (val of map_values[shortname]['values']) {
                max_val = Math.max(max_val, val['val'])
                min_val = Math.min(min_val, val['val'])
                }
            }
        } else {
            max_val = map_values['max_val']
            min_val = map_values['min_val']
        }
...
/*            var numberOfUnis = 0;
            ...
            if (region_name in map_values) {
                cc = getValByTime(map_values[region_name]['values'], time)
            } else {
                cc = getValByTime(map_values['default']['values'], time)
            }
            if (cc !== null) {
                cc /= (max_val - min_val)
                t = Math.floor(cc * (color_gradient.length - 1))
                //pp = Math.floor(cc * 255)
                //col = '#'+rgbToHex(Math.floor(pp/2)+1,Math.floor(pp/2)+1,pp)
                FSsetColor(region_name, color_gradient[t])
            } else {
                FSsetColor(region_name, no_data_color)
            }
        }
    }

    function getValByTime(values, t) {
        for (val of values) {
            if (val['from'] <= t) {
                if (val['to'] >= t)
                    return val['val'];
    ...
    function loadRegion(c) {
        ...
        currentSet.hover(function (Y) {
            ...
            toolTip(currentData.image, currentData.comment, currentData.shortname, currentData.link);
        }, function (Y) {
    ...
    function map_init() {
        ...
        let html = "<input id='range_day_night"+mapId+"' type='range' min='0' max=100 step='any' width='inherit'>";
        ...
        document.getElementById('range_day_night'+mapId).oninput = function (E) {
            this.value
            console.log(this.value/100 * 24 * 60 * 60);
            updateToTime(this.value/100 * 24 * 60 * 60);
        }
